...
Page({
  ...
  registerCheck() {
    if ((this.data.ps1 === undefined)||(this.data.account_id === undefined)) {
      wx.showToast({
        icon : 'error',
        title: '请填写信息',
      })
    ...
    else if (this.data.ps1 != this.data.ps2) {
      wx.showToast({
        icon :'error',
        title: '密码不相同',
      })
      ...
      return false
    } else if (this.data.account_id.length < 1) {
      wx.showToast({
        icon : 'error',
        title: '昵称过短',
      })
      return false
    } else if (this.data.ps1.length < 6) {
      wx.showToast({
        icon : 'error',
        title: '密码过短',
      })
